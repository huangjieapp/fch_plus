...
@interface BrokerCustomVC ()<UITableViewDelegate,UITableViewDataSource,CustomerDetailBottomToolViewDelegate,MFMessageComposeViewControllerDelegate>

@property(nonatomic,strong)UITableView*tableView;
@property(nonatomic,strong)CustomerDetailBottomToolView*toolView;
/** <#注释#> */
@property (nonatomic, strong) NSString *path0;
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self httpPostGetCustomerDetailInfoWithSuccessBlock:^{
        NSArray *codeArr = @[@"1",@"2",@"3",@"A85500_C_TYPE_0005"];
        for (NSString *code in codeArr) {
            [self httpGetCustomerPathWithType:code];
        }
...
//            
//            cell.numberArray=[@[self.mainPathModel.B_HYGH.length > 0 ? self.mainPathModel.B_HYGH : @"",self.mainPathModel.B_BB.length > 0 ? self.mainPathModel.B_BB : @""] mutableCopy];
        cell.numberArray = [@[self.path0,self.path1,self.path2,self.path3] mutableCopy];
//        }
        ...
        cell.clickTopThreeButtonBlock = ^(NSInteger index) {
            ...
            //    vc.body = @"吃饭了没？";
                ...
                if (![[NewUserSession instance].appcode containsObject:@"crm:a477_zj:edit"]) {
                    [JRToast showWithText:@"账号无权限"];
                    return ;
                }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==1) {
        ...
        if ([model.C_TYPE isEqualToString:@"协助跟进"]) {
            [self assistFollowDetailVCWithModel:model andIndexPath:indexPath];
        }
        
//        if ([model.C_TYPE_DD_ID isEqualToString:@"A85500_C_TYPE_0004"]) {
...
//
//
//      /Users/huangjie/Documents/房车汇+/房车汇PRO/Rebuild_newMJK/Rebuild_newMJK/contents/discovery/经纪人中心/BrokerOldCustomerVC.m:    self.title=@"粉丝详情";  }else{
            //不可编辑
...
-(void)httpGetCustomerPathWithsuccess:(void(^)(id data))successBlock{
    if (![[NewUserSession instance].appcode containsObject:@"crm:a477_zj:gjlist"]) {
        [JRToast showWithText:@"账号无权限"];
        return;
    }
    NSString*C_TYPE=@"1";
    ...
    if ([C_TYPE isEqualToString:@"A85500_C_TYPE_0005"]) {
        
        contentDic[@"C_TYPE_DD_ID"] = C_TYPE;
    } else {
...
-(void)httpGetCustomerPathWithType:(NSString *)C_TYPE {
    DBSelf(weakSelf);
    NSMutableDictionary *contentDic = [NSMutableDictionary dictionary];
    contentDic[@"pageNum"] = @(self.pages);
    contentDic[@"pageSize"] = @(self.pagen);
    ...
    [manager getNewDataFromNetworkNOHUDWithUrl:[NSString stringWithFormat:@"%@/api/crm/a855/list", HTTP_IP] parameters:contentDic compliation:^(id data, NSError *error) {
        if ([data[@"code"] integerValue]==200) {
            ...
            if ([C_TYPE isEqualToString:@"1"]){
                weakSelf.path0 = [NSString stringWithFormat:@"%@",data[@"data"][@"count"]];
            } else if ([C_TYPE isEqualToString:@"2"]){
                weakSelf.path1 = [NSString stringWithFormat:@"%@",data[@"data"][@"count"]];
            } else if ([C_TYPE isEqualToString:@"3"]){
                weakSelf.path2 = [NSString stringWithFormat:@"%@",data[@"data"][@"count"]];
            } else if ([C_TYPE isEqualToString:@"A85500_C_TYPE_0005"]) {
                weakSelf.path3 = [NSString stringWithFormat:@"%@",data[@"data"][@"count"]];
            }
            [weakSelf.tableView reloadData];
        }else{
            [JRToast showWithText:data[@"message"]];
...
//旧的客户轨迹
-(void)httpPostCustomerPathWithIndex:(NSInteger)index andsuccess:(void(^)(id data))successBlock{
    ...
    NSMutableDictionary*mtDict=[DBObjectTools getAddressDicWithAction:HTTP_CustomerPath];
    //A41500  没有值
    NSDictionary*contentDict=@{@"C_A41500_C_ID":self.mainModel.C_A41500_C_ID,@"C_TYPE":C_TYPE,@"currPage":@(self.pages),@"pageSize":@(self.pagen)};
    [mtDict setObject:contentDict forKey:@"content"];
    NSString*paramStr=[DBObjectTools getPostStringAddressWithMainDict:mtDict withtype:nil];
    HttpManager*manager=[[HttpManager alloc]init];
    [manager postDataFromNetworkNoHudWithUrl:paramStr parameters:nil compliation:^(id data, NSError *error) {
        ...
        if ([data[@"code"] integerValue]==200) {
            ...
            self.mainPathModel=[CustomerDetailPathModel yy_modelWithDictionary:data];
            ...
            if (index==0) {
                for (CustomerDetailPathDetailModel*subModel in self.secondSectionAllDatas) {
                    if ([subModel.C_TYPE isEqualToString:@"跟进"]) {
...
-(CustomerDetailBottomToolView *)toolView{
    ...
    if (!_toolView) {
        ...
        if ([[NewUserSession instance].appcode containsObject:@"crm:a477_zj:gj"]) {
        ...
        if ([[NewUserSession instance].appcode containsObject:@"crm:a477_zj:zp"]) {
