...
@interface MJKSalesCommentTableViewCell ()<UITableViewDelegate, UITableViewDataSource>
/** <#注释#>*/
...
@implementation MJKSalesCommentTableViewCell
...
- (void)setModel:(MJKCommentsListModel *)model {
    ...
    for (MJKCommentsListModel *subModel in model.replyList) {
...
+ (instancetype)cellWithTableView:(UITableView *)tableView {
    static NSString *ID = @"MJKSalesCommentTableViewCell";
    MJKSalesCommentTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return self.model.replyList.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MJKCommentsListModel *model = self.model.replyList[indexPath.row];
    MJKSalesCommentSUbTableViewCell *cell = [MJKSalesCommentSUbTableViewCell cellWithTableView:tableView];
...
- (MJKPhotoView *)tableCommentDetailPhoto {
    if (!_tableCommentDetailPhoto) {
        _tableCommentDetailPhoto = [[MJKPhotoView alloc]initWithFrame:CGRectMake(0, 0, KScreenWidth, 130)];
        _tableCommentDetailPhoto.isEdit = NO;
        _tableCommentDetailPhoto.isCamera = NO;
        _tableCommentDetailPhoto.rootVC = self.rootVC;
        _tableCommentDetailPhoto.titleNameStr = @"评论";
