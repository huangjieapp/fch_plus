...
-(id)init
{
    self = [super init];
    if (self) {
        self.itemSize = CGSizeMake(WIDE, 218);
    ...
    for (UICollectionViewLayoutAttributes* attributes in array) {
        if (CGRectIntersectsRect(attributes.frame, rect)) {
            ...
            if (distance < kScreen_Width / 2 + self.itemSize.width) {
                ...
                attributes.transform3D = CATransform3DTranslate(attributes.transform3D, 0 , -zoom * 0.1, 0);
...
- (CGPoint)targetContentOffsetForProposedContentOffset:(CGPoint)proposedContentOffset withScrollingVelocity:(CGPoint)velocity
{
    ...
    CGRect targetRect = CGRectMake(proposedContentOffset.x, 0.0, WIDE, 218);
