...
@interface MJKNewTaskListViewController ()<UITableViewDelegate,UITableViewDataSource,AMapLocationManagerDelegate>
...
@implementation MJKNewTaskListViewController
...
-(void)addChooseViewWithAaleModel:(MJKClueListViewModel *)saleDatasModel{
    ...
    menuView.defaulTitleArray= @[@"负责人",@"状态",@"类型",@"计划开始时间"];
    ...
    funnelView.allDatas =  (NSMutableArray *)@[dic0,dic,dic1,dic7,dic2,dic4,dic5,dic3];
    ...
    funnelView.resetBlock = ^{
        [weakSelf.saveSelTableDict removeObjectForKey:@"ISYQ"];
        [weakSelf.saveSelTableDict removeObjectForKey:@"CREATE_TIME_TYPE"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.vcName isEqualToString:@"打卡"]) {
        ...
        for (UIViewController *rootVC in self.navigationController.viewControllers) {
            if ([rootVC isKindOfClass:[MJKWorkWorldSignViewController class]]) {
...
-(void)HttpPostgetListDatas{
    ...
    if (self.C_A41500_DD_ID.length > 0) {
        contentDict[@"C_A41500_C_ID"] = self.C_A41500_DD_ID;
    }
