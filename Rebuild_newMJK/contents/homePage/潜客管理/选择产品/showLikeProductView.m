...
-(void)getShowValue:(NSMutableArray*)allDatas{
    ...
    for (int i=0; i<self.datasArray.count; i++) {
        ...
        [addButton addTarget:self action:@selector(changeNumber:) forControlEvents:UIControlEventTouchUpInside];
...
//        numLab.text=model.B_PRICE;
        ...
        [minusButton addTarget:self action:@selector(changeNumber:) forControlEvents:UIControlEventTouchUpInside];
        ...
        [deleteButton addTarget:self action:@selector(clickDeleteOneInfo:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)changeNumber:(UIButton *)btn{
    ...
    if ([btn.titleNormal isEqualToString:@"+"]) {
        self.number+=1;
        if (self.addBtnBlock) {
            self.addBtnBlock();
        }
