...
@interface MJKQualityAssuranceListViewController ()<UITableViewDataSource, UITableViewDelegate>
/** <#注释#> */
...
@implementation MJKQualityAssuranceListViewController
...
- (void)viewDidLoad {
    ...
    [self httpGetUserListWithModelArrayBlock:^(NSArray *saleArray) {
        @strongify(self);
        [self geta800DatasCompliationWithC_TYPE_DD_ID:@"A80000_C_TYPE_0003" andBlock:^(NSArray *deployDatas) {
        ...
        for (MJKAdditionalInfoModel* model in deployDatas) {
            [mtArray addObject:model.C_NAME.length > 0 ? model.C_NAME : @""];
            [postArray addObject:model.C_ID.length > 0 ? model.C_ID : @""];
        }
        
        NSMutableArray*statusArray=[[FMDBManager sharedFMDBManager]queryDatasWithC_TYPECODE:@"A80400_C_STATUS"];
        ...
        self.dropDownMenuView.defaulTitleArray = [NSArray arrayWithObjects:@"销售",@"车型",@"质保公司", @"状态", nil];
        ...
        self.dropDownMenuView.chooseConditionBlock = ^(NSString *selectedSection, NSString *selectedRow, NSString *title) {
            ...
            if(selectedSection.intValue == 0) {
                ...
                self.saveTableDataDic[@"C_A80000ZB_C_ID"] = postArray[selectedRow.intValue];
...
//    contentDic[@"C_LOCCODE"] = [NewUserSession instance].user.C_LOCCODE;
    HttpManager*manager=[[HttpManager alloc]init];
    [manager postNewDataFromNetworkNoHudWithUrl:[NSString stringWithFormat:@"%@/api/crm/a804/list", HTTP_IP] parameters:contentDic compliation:^(id data, NSError *error) {
        MyLog(@"%@",data);
        if ([data[@"code"] integerValue] == 200) {
            weakSelf.naviCountView.countLabel.text = [NSString stringWithFormat:@"总计:%@",data[@"data"][@"countNumber"]];
            weakSelf.dataArray = [MJKQualityAssuranceMainModel mj_objectArrayWithKeyValuesArray:data[@"data"][@"content"]];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    MJKQualityAssuranceMainModel *model = self.dataArray[section];
    return model.content.count;
}


- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MJKQualityAssuranceMainModel *model = self.dataArray[indexPath.section];
    MJKQualityAssuranceModel *subModel = model.content[indexPath.row];
    ...
    if (subModel.C_A41500_C_NAME.length > 0) {
        cell.headLabel.text = [subModel.C_A41500_C_NAME substringToIndex:1];
    } else {
        cell.headLabel.text = @"";
    }
    cell.titleLabel.text = subModel.C_A41500_C_NAME;
    cell.rightLabel.text = subModel.C_CP;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (![[NewUserSession instance].appcode containsObject:@"crm:a804:detail"]) {
