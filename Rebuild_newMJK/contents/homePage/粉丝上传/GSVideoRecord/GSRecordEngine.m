...
#import "GSRecordEncoder.h"
#import <AVFoundation/AVFoundation.h>
#import <Photos/Photos.h>

@interface GSRecordEngine () <AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureAudioDataOutputSampleBufferDelegate, CAAnimationDelegate>
{
...
@property (strong, nonatomic) GSRecordEncoder           *recordEncoder;//录制编码
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.maxRecordTime = GS_Video_Limit_Seconds;
    }
    return self;
}
...
- (dispatch_queue_t)captureQueue {
    if (_captureQueue == nil) {
        _captureQueue = dispatch_queue_create("cn.qiuyouqun.im.GSrecordengine.capture", DISPATCH_QUEUE_SERIAL);
    }
...
- (void) captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    @synchronized(self) {
        ...
        if ((self.recordEncoder == nil) && !isVideo) {
            ...
            self.recordEncoder = [GSRecordEncoder encoderForPath:self.videoPath Height:_cy width:_cx channels:_channels samples:_samplerate];
        }
        //判断是否中断录制过
        if (self.discont) {
            if (isVideo) {
...
{
    if (_videoPath) {
        [[NSFileManager defaultManager] removeItemAtPath:_videoPath error:nil];//删除app的
