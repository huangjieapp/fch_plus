...
- (void)playerControlView:(UIView *)controlView playerModel:(SLPlayerModel *)playerModel {
    ...
    [self updateCloseImageViewWithFull:YES];
}
...
- (void)addPlayerDynamicListToFatherView:(UIView *)fatherView EdgesView:(UIView *)edgesView {
    if (fatherView) {
        [self removeFromSuperview];
        [fatherView addSubview:self];
        [self mas_makeConstraints:^(MASConstraintMaker *make) {
               make.top.mas_equalTo(edgesView);
               make.left.mas_equalTo(edgesView);
               make.right.mas_equalTo(edgesView);
               make.bottom.mas_equalTo(edgesView);
        }];
    }
}
    ...
    if (self.isCellVideo) {
        if ([self.scrollView isKindOfClass:[UITableView class]]) {
            ...
            if (![tableView.visibleCells containsObject:cell]) {
                ...
                if ([cell isKindOfClass:[GSPSVideoCell class]]) {
                    GSPSVideoCell *headerCell = (GSPSVideoCell *)cell;
                    UIView *fatherView = headerCell.maskV;
                    [self addPlayerDynamicListToFatherView:fatherView EdgesView:fatherView];
                } else {
...
- (void)_fullScreenAction {
    ...
    if (self.isFullScreen) {
        [self interfaceOrientation:UIInterfaceOrientationPortrait];
        self.isFullScreen = NO;
        [self updateCloseImageViewWithFull:NO];
        return;
    } else {
        UIDeviceOrientation orientation = [UIDevice currentDevice].orientation;
...
- (void)updateCloseImageViewWithFull:(BOOL)full {
    float the_top = 0;
    float the_bottom = -5;
    if (full) {
        the_top = 15;
    ...
    UIImageView *topIV = ((SLPlayerControlView *)self.controlView).topImageView;
    if (topIV) {
        UIButton *backBtn = ((SLPlayerControlView *)self.controlView).backBtn;
        if (backBtn) {
            float thedev = the_top;
            [backBtn mas_remakeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(topIV.mas_top).offset(thedev);
                make.left.equalTo(topIV.mas_left).offset(5);
                make.width.height.mas_equalTo(50);
            }];
        }
    }
    
    UIButton *startButton = ((SLPlayerControlView *)self.controlView).startBtn;
    if (startButton) {
        UIImageView *bottomIV = ((SLPlayerControlView *)self.controlView).bottomImageView;
        [startButton mas_updateConstraints:^(MASConstraintMaker *make) {
            make.bottom.equalTo(bottomIV.mas_bottom).offset(the_bottom);
        }];
    }
}
