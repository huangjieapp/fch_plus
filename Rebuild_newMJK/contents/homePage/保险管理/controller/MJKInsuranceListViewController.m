...
@interface MJKInsuranceListViewController ()<UITableViewDataSource, UITableViewDelegate>
/** <#注释#> */
...
@implementation MJKInsuranceListViewController
...
- (void)viewDidLoad {
    ...
    [self httpGetUserListWithModelArrayBlock:^(NSArray *saleArray) {
        @strongify(self);
        [self geta800DatasCompliationWithC_TYPE_DD_ID:@"A80000_C_TYPE_0000" andBlock:^(NSArray *deployDatas) {
        ...
        NSMutableArray*statusArray=[[FMDBManager sharedFMDBManager]queryDatasWithC_TYPECODE:@"A80300_C_STATUS"];
        ...
        self.dropDownMenuView.defaulTitleArray = [NSArray arrayWithObjects:@"销售",@"车型",@"保险公司", @"状态", nil];
        ...
        self.dropDownMenuView.chooseConditionBlock = ^(NSString *selectedSection, NSString *selectedRow, NSString *title) {
            ...
            if(selectedSection.intValue == 0) {
                ...
                self.saveTableDataDic[@"C_A80000BX_C_ID"] = postArray[selectedRow.intValue];
...
//    contentDic[@"C_LOCCODE"] = [NewUserSession instance].user.C_LOCCODE;
    HttpManager*manager=[[HttpManager alloc]init];
    [manager postNewDataFromNetworkNoHudWithUrl:[NSString stringWithFormat:@"%@/api/crm/a803/list", HTTP_IP] parameters:contentDic compliation:^(id data, NSError *error) {
        MyLog(@"%@",data);
        if ([data[@"code"] integerValue] == 200) {
            weakSelf.naviCountView.countLabel.text = [NSString stringWithFormat:@"总计:%@",data[@"data"][@"countNumber"]];
            weakSelf.dataArray = [MJKInsuranceMainModel mj_objectArrayWithKeyValuesArray:data[@"data"][@"content"]];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    MJKInsuranceMainModel *model = self.dataArray[section];
    return model.content.count;
}


- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MJKInsuranceMainModel *model = self.dataArray[indexPath.section];
    MJKInsuranceModel *subModel = model.content[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (![[NewUserSession instance].appcode containsObject:@"crm:a803:detail"]) {
