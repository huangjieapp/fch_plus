...
@interface CustomerFollowAddEditViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,IFlySpeechRecognizerDelegate,BMKLocationServiceDelegate,BMKGeoCodeSearchDelegate>
...
/** <#注释#>*/
@property (nonatomic, strong) NSArray *btNormalArray;
/** <#注释#>*/
...
@implementation CustomerFollowAddEditViewController
...
- (void)viewDidLoad {
    ...
    if (self.Type==CustomerFollowUpAdd) {
        self.title=@"客户跟进添加";
        if (self.forRemark.length <=0) {
            if (self.infoModel.X_REMRAK.length > 0) {
                CustomerFollowModel*model=self.saveAllModelArray[1];
                model.showStr = self.infoModel.X_REMRAK;
                model.postStr = self.infoModel.X_REMRAK;
            }
        }
    ...
    if (self.Type == CustomerFollowUpNil) {
        
    } else if (/*(self.Type == CustomerFollowUpEdit && self.canEdit == YES) ||*/ self.Type == CustomerFollowUpAdd) {
    ...
    self.btNormalArray = [NewUserSession instance].configData.btListMapKh;
...
- (void)didUpdateBMKUserLocation:(BMKUserLocation *)userLocation {
    ...
    if (self.DatasMainModel.B_DRIVE_LAT.length > 0 && self.DatasMainModel.B_DRIVE_LON.length > 0) {
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    NSInteger sfzbIndex = 0;
    for (int i = 0; i < self.saveAllModelArray.count; i++) {
        CustomerFollowModel *tempModel = self.saveAllModelArray[i];
        if ([tempModel.titleStr isEqualToString:@"是否战败"]) {
            sfzbIndex = i;
        }
    }
        CustomerFollowModel *model=self.saveAllModelArray[sfzbIndex];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([model.titleStr isEqualToString:@"客户姓名"]) {
        //
        ...
        if (model.showStr.length <= 0) {
            cell.followImage.hidden = YES;
        } else {
        ...
        //            if ([self.btArray containsObject:@"A47500_C_BTX_0011"]) {
        ...
        return cell;
    }else if ([model.titleStr isEqualToString:@"跟进方式"]){
        ...
        cell.Type=ChooseTableViewTypeMODEFOLLOW;
        cell.chooseBlock = ^(NSString *str, NSString *postValue) {
            MyLog(@"str-- %@      post---%@",str,postValue);
            if ([postValue isEqualToString:@"A41600_C_MODEFOLLOW_0004"] || [postValue isEqualToString:@"A41600_C_MODEFOLLOW_0009"]) {
        ...
        return cell;
    } else if ([model.titleStr isEqualToString:@"跟进等级"]){
        AddCustomerChooseTableViewCell*cell=[tableView dequeueReusableCellWithIdentifier:CELL2];
        cell.nameTitleLabel.text=model.titleStr;
        if (model.showStr&&![model.showStr isEqualToString:@""]) {
            cell.textStr=model.showStr;
        ...
        cell.chooseBlock = ^(NSString *str, NSString *postValue) {
            ...
            for (int i = 0; i < self.saveAllModelArray.count; i++) {
                ...
                if ([tempModel.titleStr isEqualToString:@"是否有效跟进"]) {
                    sfyxIndex = i;
                }
                ...
                CustomerFollowModel *dateModel4=weakSelf.saveAllModelArray[sfyxIndex];
            if ([dateModel4.postStr isEqualToString:@"1"]) {
                
                //得到跟进的值 并刷新
                [weakSelf getLaterTimerWithLevel:model.showStr andCustomTime:dateModel.showStr];
            } else {
                    CustomerFollowModel *dateModel7=weakSelf.saveAllModelArray[xcIndex];
                //得到跟进的值 并刷新
                dateModel7.showStr = [DBTools getTimeFomatFromTimeStampAddDay:2 andTime:dateModel.showStr];
                dateModel7.postStr= [DBTools getTimeFomatFromTimeStampAddDay:2 andTime:dateModel.showStr];
        ...
        cell.switchButtonActionBlock = ^{
            if ([model.postStr isEqualToString:@"1"]) {
                model.postStr = @"0";
//                CustomerFollowModel*dateModel5;
                NSInteger sjIndex = 0;
                NSInteger xcIndex = 0;
                for (int i = 0; i < weakSelf.saveAllModelArray.count; i++) {
                    ...
                    CustomerFollowModel *dateModel5=weakSelf.saveAllModelArray[sjIndex];
                    CustomerFollowModel *dateModel7=weakSelf.saveAllModelArray[xcIndex];
                //得到跟进的值 并刷新
                dateModel7.showStr = [DBTools getTimeFomatFromTimeStampAddDay:2 andTime:dateModel5.showStr];
                dateModel7.postStr= [DBTools getTimeFomatFromTimeStampAddDay:2 andTime:dateModel5.showStr];
                [tableView reloadData];
            } else {
                model.postStr = @"1";
                NSInteger sjIndex = 0;
                NSInteger gjdjIndex = 0;
                for (int i = 0; i < self.saveAllModelArray.count; i++) {
                    CustomerFollowModel *tempModel = self.saveAllModelArray[i];
                    if ([tempModel.titleStr isEqualToString:@"实际跟进时间"]) {
                        sjIndex = i;
                    }
                    if ([tempModel.titleStr isEqualToString:@"跟进等级"]) {
                        gjdjIndex = i;
                    }
                }
                    CustomerFollowModel *dateModel3=weakSelf.saveAllModelArray[gjdjIndex];
                    CustomerFollowModel *dateModel=weakSelf.saveAllModelArray[sjIndex];
                //得到跟进的值 并刷新
                [weakSelf getLaterTimerWithLevel:dateModel3.showStr andCustomTime:dateModel.showStr];
            }
        ...
        cell.chooseBlock = ^(NSString *str, NSString *postValue) {
            ...
            if (result == 1) {
                ...
                for (int i = 0; i < self.saveAllModelArray.count; i++) {
                    ...
                    CustomerFollowModel *dateModel4=weakSelf.saveAllModelArray[sfyxIndex];
                if ([dateModel4.postStr isEqualToString:@"1"]) {
                        CustomerFollowModel *dateModel=weakSelf.saveAllModelArray[gjdjIndex];
                    //得到跟进的值 并刷新
                    [weakSelf getLaterTimerWithLevel:dateModel.showStr andCustomTime:model.showStr];
                } else {
                        CustomerFollowModel *dateModel7=weakSelf.saveAllModelArray[xcIndex];
                    //得到跟进的值 并刷新
                    dateModel7.showStr = [DBTools getTimeFomatFromTimeStampAddDay:2 andTime:model.showStr];
                    dateModel7.postStr= [DBTools getTimeFomatFromTimeStampAddDay:2 andTime:model.showStr];
                    [tableView reloadData];
                }
    ...
    }else if ([model.titleStr isEqualToString:@"是否战败"]){
        //        UITableViewCell*cell=[tableView dequeueReusableCellWithIdentifier:@"cell"];
        //        if (!cell) {
        //            cell=[[UITableViewCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"cell"];
        ...
        ZhanbaiChooseTableViewCell*cell=[tableView dequeueReusableCellWithIdentifier:zhanbaiCELL];
        cell.titleStr=model.titleStr;
        if ([model.showStr isEqualToString:@"是"]) {
            cell.number=1;
        }else{
            cell.number=0;
        }
        if ([self.title isEqualToString:@"客户跟进详情"]) {
            //            cell.tureButton.enabled = cell.falseButton.enabled = NO;
            cell.userInteractionEnabled = NO;
        }
        if (self.Type != CustomerFollowUpAdd) {
            cell.falseButton.enabled = cell.tureButton.enabled = NO;
        }
        cell.clickSelectedButtonBlock = ^(NSInteger number) {
            //否
            NSInteger sfzbIndex = 0;
            for (int i = 0; i < self.saveAllModelArray.count; i++) {
                CustomerFollowModel *tempModel = self.saveAllModelArray[i];
        ...
        //        UISwitch*thisSwitch=[[UISwitch alloc]initWithFrame:CGRectMake(KScreenWidth-100+15, 7, 50, 30)];
        //        thisSwitch.onTintColor=DBColor(237, 84, 84);
        //        [thisSwitch addTarget:self action:@selector(touchSwitch:) forControlEvents:UIControlEventValueChanged];
        //        [cell addSubview:thisSwitch];
        //
        //        UILabel*rightLabel=[[UILabel alloc]initWithFrame:CGRectMake(thisSwitch.x+thisSwitch.width+5, 7, 20, 30)];
        //        rightLabel.font=[UIFont systemFontOfSize:14];
        //        rightLabel.textColor=[UIColor colorWithHexString:@"#999999"];
        //        self.switchLabel=rightLabel;
...
-(UIView*)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    ...
    /*  CGCOrderDetialFooter*footer=[[NSBundle mainBundle]loadNibNamed:@"CGCOrderDetialFooter" owner:nil options:nil].firstObject;
     ...
     return footer;
     */
    if (self.Type == CustomerFollowUpAdd /*|| (self.Type == CustomerFollowUpEdit && self.canEdit == YES)*/) {
...
//    self.tableFootPhoto.imageURLArray = self.DatasMainModel.urlList;
    if (self.DatasMainModel.fileList.count <= 0 && self.Type != CustomerFollowUpAdd) {
        return nil;
    }else {
...
//    }
//
//    CustomerFollowModel*model=self.saveAllModelArray[5];
...
//先判断是否战败  不管是否战败 都接下来 图片和提交跟进
-(void)cheatZhanbaiSuccess:(void(^)(void))successBlock andFail:(void(^)())failBlock{
    DBSelf(weakSelf);
    //先搞战败 完成了 传图片   在新增潜客
    NSInteger sfzbIndex = 0;
    for (int i = 0; i < self.saveAllModelArray.count; i++) {
    ...
    if ([model.showStr isEqualToString:@"是"]) {
        ...
        CGCAlertDateView *alertDate=[[CGCAlertDateView alloc] initWithFrame:self.view.bounds  withSelClick:^{
            ...
            if (dateStr.length>0) {
                ...
                [MJKHttpApproval DefeatGetHttpValuesWithObjectId:self.infoModel.C_ID andX_REMARK:dateStr andC_REMARK_TYPE_DD_ID:failChooseCodeArray[index] andSuccessBlock:^{
                    successBlock();
                }];
            }else{
...
-(void)httpPostFollowWithPhotoStr:(NSArray*)imageArray success:(void(^)(id data))successBlock{
    ...
    CustomerFollowModel*dateModel2=self.saveAllModelArray[2];
    if ([dateModel2.postStr isEqualToString:@"A41600_C_MODEFOLLOW_0009"] || [dateModel2.postStr isEqualToString:@"A41600_C_MODEFOLLOW_0004"]) {
        ...
        if (self.locationAddress.length > 0) {
            [contentDict setObject:self.locationAddress forKey:@"C_DRIVE_ADDRESS"];
        }
    }
...
//    NSString*encodeStr=[DBObjectTools getPostStringAddressWithMainDict:mainDict withtype:nil];
    HttpManager*manager=[[HttpManager alloc]init];
    [manager postNewDataFromNetworkWithUrl:HTTP_AddCustomerFollow parameters:contentDict compliation:^(id data, NSError *error) {
        MyLog(@"%@",data);
        if ([data[@"code"] integerValue]==200) {
...
//            }
            ...
            NSArray*showArray;
            NSArray*postArray;
                showArray=@[model.C_A41500_C_NAME.length > 0 ? model.C_A41500_C_NAME : @"",
            ...
            if ([model.C_MODEFOLLOW_DD_ID isEqualToString:@"A41600_C_MODEFOLLOW_0009"] || [model.C_MODEFOLLOW_DD_ID isEqualToString:@"A41600_C_MODEFOLLOW_0004"]) {
                ...
                [self.saveAllModelArray insertObject:addModel atIndex:3];
            }
...
#pragma 提交的时候  是否战败 这个字段不传。  但是要判断是否需要战败
    ...
    NSArray*postArray;
    NSArray*keyArray;
        titleArray=@[@"客户姓名",@"跟进内容",@"跟进方式",@"跟进等级",@"是否有效跟进",@"实际跟进时间",@"是否战败",@"下次跟进时间"];
    showArray=@[self.infoModel.C_NAME.length > 0?self.infoModel.C_NAME:@"",
                self.followText,
                self.infoModel.C_MODEFOLLOW_DD_NAME.length > 0?self.infoModel.C_MODEFOLLOW_DD_NAME:@"",
                self.infoModel.C_LEVEL_DD_NAME.length > 0?self.infoModel.C_LEVEL_DD_NAME:@"",
                self.Type == CustomerFollowUpAdd ? @"1" : ( self.infoModel.I_SORTIDX?self.infoModel.I_SORTIDX:@"1"),
                [DBTools getTimeFomatFromCurrentTimeStamp],
                @"否",
                @""];
    postArray=@[self.infoModel.C_NAME?self.infoModel.C_NAME:@"",
                self.followText,
                self.infoModel.C_MODEFOLLOW_DD_ID?self.infoModel.C_MODEFOLLOW_DD_ID:@"",
                self.infoModel.C_LEVEL_DD_ID?self.infoModel.C_LEVEL_DD_ID:@"",
                self.Type == CustomerFollowUpAdd ? @"1" : (self.infoModel.I_SORTIDX?self.infoModel.I_SORTIDX:@"1"),
                showArray[5],
                @"否",
                @""];
        keyArray=@[@"C_A41500_C_NAME",@"X_REMARK",@"C_MODEFOLLOW_DD_ID",@"C_LEVEL_DD_ID",@"I_SORTIDX",@"D_FOLLOW_TIME",@"",@"D_NEXTFOLLOW_TIME"];
...
-(void)getLaterTimerWithLevel:(NSString*)customLV andCustomTime:(NSString*)currentTime{
    ...
    for (CustomerLvevelNextFollowModel*TimerModel in self.saveTimeNumberArray) {
        ...
        if ([TimerModel.C_NAME isEqualToString:str]) {
            ...
            NSString*lastTimer=[DBTools getTimeFomatFromTimeStampAddDay:number andTime:currentTime];
...
-(NSString*)judgeCanSave{
    ...
    for (int i = 0; i < self.saveAllModelArray.count; i++) {
        ...
        if ([tempModel.titleStr isEqualToString:@"跟进方式"]) {
            gjfsIndex = i;
        }
    ...
    for (NSDictionary *dic in self.btNormalArray) {
        if ([dic[@"CODE"] isEqualToString:@"A47500_C_BTX_0011"]) {
            if (model.showStr.length <= [dic[@"NUMBER"] integerValue]) {
                _canSave=NO;
                return [NSString stringWithFormat:@"跟进内容不得少于%@个字",dic[@"NUMBER"]];
            }
        }
    }
    
    
   model=self.saveAllModelArray[gjfsIndex];
    if (!model.showStr||[model.showStr isEqualToString:@""]) {
        _canSave=NO;
        return @"请选择跟进方式";
        ...
        model=self.saveAllModelArray[gjdjIndex];
    if (!model.showStr||[model.showStr isEqualToString:@""]) {
        _canSave=NO;
        return @"请选择跟进等级";
