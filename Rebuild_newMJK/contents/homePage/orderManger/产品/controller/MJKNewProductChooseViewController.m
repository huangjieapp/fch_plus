...
#import "MJKShoppingCartMoedel.h"
...
- (void)getActivityCategory {
    ...
    [manager postDataFromNetworkNoHudWithUrl:encodeUrlStr parameters:nil compliation:^(id data, NSError *error) {
        ...
        if ([data[@"code"] integerValue]==200) {
            weakSelf.categoryArray = [CustomerLvevelNextFollowModel mj_objectArrayWithKeyValuesArray:data[@"content"]];
...
- (void)HTTPGetProductList {//获取订单列表
    DBSelf(weakSelf);
    NSMutableDictionary*dict=[DBObjectTools getAddressDicWithAction:@"A49600WebService-getList"];
    ...
    if (self.isSearch == YES) {
        contentDic[@"X_REMARK"] = self.searchStr;
    } else {
        contentDic[@"C_TYPE_DD_ID"] = self.C_TYPE_DD_ID;
    }
    if (self.paramDic == contentDic) {
        [self.mainTabelView.mj_header endRefreshing];
        [self.mainTabelView.mj_header endRefreshing];
        return;
    }
    self.paramDic = contentDic;
    ...
    [manager postDataFromNetworkNoHudWithUrl:encodeUrlStr parameters:nil compliation:^(id data, NSError *error) {
        if ([data[@"code"] integerValue]==200) {
            weakSelf.mainShowArray = [MJKProductShowModel mj_objectArrayWithKeyValuesArray:data[@"content"]];
            if (self.isSearch == YES) {
