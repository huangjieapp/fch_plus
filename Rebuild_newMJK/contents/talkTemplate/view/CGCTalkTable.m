...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_style==CGCTalkTableText) {
        CGCTalkModel *model=(self.dataArray.count>indexPath.section)?self.dataArray[indexPath.section]:nil;
...
- (void)tableScrollClick:(NSIndexPath *)indexPath withTitle:(NSString *)title{
    if ([self.delegate respondsToSelector:@selector(talkTable:didClickEidtIndex:withEidt:withTitle:)]) {
        [self.delegate talkTable:self didClickEidtIndex:indexPath withEidt:_eidtStyle withTitle:title];
    }
...
- (UIView *)headSelView{
    ...
    if (!_headSelView) {
        ...
        for (int i=0; i<self.headTitleArr.count; i++) {
            ...
            [btn setTitle:self.headTitleArr[i]];
...
- (void)headSelClick:(UIButton *)button{
    ...
    if ([self.delegate respondsToSelector:@selector(talkTable:didClickWithTitle:)]&&![self.selBtnStr isEqualToString:button.title]) {
        self.selBtnStr=button.title;
        [self.delegate talkTable:self didClickWithTitle:button.title];
    }
