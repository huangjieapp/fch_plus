//
//  MJKOrderInfoOpenViewController.m
//  Rebuild_newMJK
//
...
#import "MJKOrderInfoOpenViewController.h"

#import "MJKCustomReturnSubModel.h"

#import "MJKRecheckTableViewCell.h"

@interface MJKOrderInfoOpenViewController ()<UITableViewDataSource, UITableViewDelegate>
/** tableView*/
...
@implementation MJKOrderInfoOpenViewController
...
- (void)viewDidLoad {
    ...
    self.title = @"订单信息开放选择设置";
    [self initUI];
}

- (void)initUI {
    [self.view addSubview:self.tableView];
    [self HTTPRecheckSetInfoOpenDatas];
}
...
//MARK:-http
- (void)HTTPRecheckSetInfoOpenDatas {
    DBSelf(weakSelf);
...
-(void)httpSetRecheck:(MJKCustomReturnSubModel *)model {
    ...
    [manager postDataFromNetworkNoHudWithUrl:encodeUrlStr parameters:nil compliation:^(id data, NSError *error) {
        MyLog(@"%@",data);
        if ([data[@"code"] integerValue]==200) {
            NSMutableArray *arr = [NewUserSession instance].configData.ddxxkfxzseList;
