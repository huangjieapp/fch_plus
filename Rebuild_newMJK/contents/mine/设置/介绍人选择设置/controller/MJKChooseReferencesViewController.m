...
#import "MJKChooseReferencesViewController.h"
#import "MJKCustomReturnSubModel.h"

#import "MJKRecheckTableViewCell.h"

@interface MJKChooseReferencesViewController ()<UITableViewDataSource, UITableViewDelegate>
/** <#注释#>*/
...
@implementation MJKChooseReferencesViewController
...
- (void)viewDidLoad {
    ...
    self.title = @"介绍人选择设置";
    [self HTTPChooseReferencesDatas];
...
- (void)HTTPChooseReferencesDatas {
    DBSelf(weakSelf);
...
-(void)httpSetRecheck:(MJKCustomReturnSubModel *)model {
    ...
    [manager postDataFromNetworkNoHudWithUrl:encodeUrlStr parameters:nil compliation:^(id data, NSError *error) {
        MyLog(@"%@",data);
        if ([data[@"code"] integerValue]==200) {
            if ([model.C_NAME isEqualToString:@"介绍人是否开放选择"]) {
                if ([model.C_STATUS_DD_ID isEqualToString:@"A47500_C_STATUS_0000"]) {
                    [NewUserSession instance].configData.IS_JSRSFKFXZ = @"1";
                } else if ([model.C_STATUS_DD_ID isEqualToString:@"A47500_C_STATUS_0001"]) {
                    [NewUserSession instance].configData.IS_JSRSFKFXZ = @"0";
                }
            }
