...
//        make.width.mas_equalTo(30);
    ...
    self.selectLb.text=self.titleStr;
...
//    [self selectStr];
//    self.selectStr;
    self.array.count>0?[self.pickerV selectRow:[self indexOfNSArray:[self.array firstObject] WithStr:self.selectStr] inComponent:0 animated:YES]:0;
...
- (void)setSelectStr:(NSString *)selectStr{
    ...
    switch (self.arrayType) {
        ...
        case GenderArray:{
            ...
            break;
        case sellType:{
            NSArray *sellArr = [self.array objectAtIndex:0];
            
            [self.pickerV selectRow:[self indexOfNSArray:sellArr WithStr:selectStr] inComponent:0 animated:YES];
        }
            break;
        case HeightArray:{
...
- (void)GenderArray{
    ...
    for (MJKDataDicModel *model in[[FMDBManager sharedFMDBManager]queryDatasWithC_TYPECODE:@"A41500_C_SEX"] ) {
        model.C_NAME.length>0?[sexArr  addObject:model.C_NAME]:0;
    }
    [self.array addObject:sexArr];
    
//    [self.array addObject:@[@"男",@"女"]];
}
- (void)sellArray{
    
    self.selectLb.text = @"选择员工";
    [self.array addObject:self.sellNameArr];
...
- (void)completeBtnClick{
    ...
    if (self.arrayType == DeteArray) {
        ...
        for (int i = 0; i < self.array.count; i++) {
            ...
            currentNumber=[self.pickerV selectedRowInComponent:i];
    ...
    if (self.clickCompleteBlock) {
        self.clickCompleteBlock(fullStr, currentNumber);
    }
    ...
    if ([self.delegate respondsToSelector:@selector(Picker:SelectorIndixString:)]) {
        [self.delegate Picker:self SelectorIndixString:fullStr];
    }
