...
@interface MJKPhotoBrowser () <UIScrollViewDelegate, UIViewControllerTransitioningDelegate, CAAnimationDelegate> {
...
@implementation MJKPhotoBrowser

// MAKR: - Initializer

+ (instancetype)browserWithPhotoItems:(NSArray<KSPhotoItem *> *)photoItems selectedIndex:(NSUInteger)selectedIndex {
    MJKPhotoBrowser *browser = [[MJKPhotoBrowser alloc] initWithPhotoItems:photoItems selectedIndex:selectedIndex];
    return browser;
}
...
- (instancetype)initWithPhotoItems:(NSArray<KSPhotoItem *> *)photoItems selectedIndex:(NSUInteger)selectedIndex {
    ...
    if (self) {
        ...
        _dismissalStyle = MJKPhotoBrowserInteractiveDismissalStyleRotation;
        _pageindicatorStyle = MJKPhotoBrowserPageIndicatorStyleDot;
        _backgroundStyle = MJKPhotoBrowserBackgroundStyleBlurPhoto;
        _loadingStyle = MJKPhotoBrowserImageLoadingStyleIndeterminate;
...
- (void)viewDidLoad {
    ...
    if (_pageindicatorStyle == MJKPhotoBrowserPageIndicatorStyleDot) {
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (_backgroundStyle == MJKPhotoBrowserBackgroundStyleBlur) {
        [self blurBackgroundWithImage:[self screenshot] animated:NO];
    } else if (_backgroundStyle == MJKPhotoBrowserBackgroundStyleBlurPhoto) {
...
- (void)configItemViews {
    ...
    if (page != _currentPage && _presented && (page >= 0 && page < _photoItems.count)) {
        ...
        if (_backgroundStyle == MJKPhotoBrowserBackgroundStyleBlurPhoto) {
...
- (void)configPhotoView:(KSPhotoView *)photoView withItem:(KSPhotoItem *)item {
    [photoView setItem:item determinate:(_loadingStyle == MJKPhotoBrowserImageLoadingStyleDeterminate)];
}
...
- (void)swipeAction:(UISwipeGestureRecognizer *)swipe {
    if (swipe.direction == UISwipeGestureRecognizerDirectionUp) {
        if (self.swipeUpActionBlock) {
            self.swipeUpActionBlock();
        }
...
- (void)didPan:(UIPanGestureRecognizer *)pan {
    ...
    switch (_dismissalStyle) {
        case MJKPhotoBrowserInteractiveDismissalStyleRotation:
            [self performRotationWithPan:pan];
            break;
        case MJKPhotoBrowserInteractiveDismissalStyleScale:
            [self performScaleWithPan:pan];
            break;
        case MJKPhotoBrowserInteractiveDismissalStyleSlide:
...
- (void)showCancellationAnimation {
    ...
    if (_bounces && _dismissalStyle == MJKPhotoBrowserInteractiveDismissalStyleScale) {
        [UIView animateWithDuration:kSpringAnimationDuration delay:0 usingSpringWithDamping:0.6 initialSpringVelocity:0 options:kNilOptions animations:^{
