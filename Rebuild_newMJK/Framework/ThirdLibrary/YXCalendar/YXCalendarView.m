...
#define kTextDarkGrayColor  [UIColor blackColor]
...
- (void)settingHeadLabel {
    ...
    _yearMonthL = [[UILabel alloc] initWithFrame:CGRectMake((KScreenWidth - 100) / 2, 0, 100, yearMonthH)];
    _yearMonthL.text = [[YXDateHelpObject manager] getStrFromDateFormat:@"yyyy年MM月" Date:[NSDate date]];
    _yearMonthL.textAlignment = NSTextAlignmentCenter;
    _yearMonthL.textAlignment = NSTextAlignmentCenter;
    _yearMonthL.font = [UIFont systemFontOfSize:15];
    [self addSubview:_yearMonthL];
    
    UIButton *leftButton = [[UIButton alloc]initWithFrame:CGRectMake(CGRectGetMinX(_yearMonthL.frame) - 20, 0, 10, yearMonthH)];
    ...
    UIButton *rightButton = [[UIButton alloc]initWithFrame:CGRectMake(CGRectGetMaxX(_yearMonthL.frame) + 10, 0, 10, yearMonthH)];
    ...
    leftButton.titleLabel.textAlignment = rightButton.titleLabel.textAlignment = NSTextAlignmentCenter;
    ...
    for (int i = 0; i < 7; i++) {
        ...
        weekL.font = [UIFont systemFontOfSize:12];
		weekL.textColor = kTextDarkGrayColor;
...
//        weakSelf.selectDate = selDate;
    ...
    NSString *monthStr =[[YXDateHelpObject manager] getStrFromDateFormat:@"yyyy-MM-" Date:_currentDate];
    NSString *dateStr = [NSString stringWithFormat:@"%@%@日",monthStr,dayStr];
    
    if (self.buttonMonthBlock) {
        self.buttonMonthBlock(dateStr);
    }
//        [weakSelf setData];
//    };
}
