...
- (id)initDropdownWithButtonTitles:(NSArray*)titles andLeftListArray:(NSArray*)leftArray andRightListArray:(NSArray *)rightArray {
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(dismissRightView) name:@"dismissRightPart" object:nil];
        
        
        rightPartV=[RightPartView creatPartViewWithDatas:nil andSuperView:self.view];
        [self.view addSubview:rightPartV];
        rightPartV.hidden=YES;
