...
-(void)setStarNumber:(NSInteger)starNumber{
    ...
    if([delegate respondsToSelector:@selector(setRating:isHuman:andTag:)]){
        [delegate setRating:_starNumber isHuman:NO andTag:self.tag];
    }
}
-(void)tap:(UITapGestureRecognizer *)gesture{
    ...
    if([delegate respondsToSelector:@selector(setRating:isHuman:andTag:)]){
        [delegate setRating:_starNumber isHuman:YES andTag:self.tag];
    }
}
-(void)pan:(UIPanGestureRecognizer *)gesture{
    if(self.enable){
        ...
        if(count>=0 && count<=10 && _starNumber!=count){
